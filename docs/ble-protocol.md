...
**역할**: 펌웨어(v3.2 XIAO nRF52840 / v4.0 ESP32-S3)와 Flutter 앱 간 단일 진실의 원천. 양측 구현은 이 문서를 기준으로 한다.
**변경 규칙**: 이 문서를 수정하면 PR에 양측 담당자(컴공·시디) 리뷰 필수.

> **v4.0 호환 노트** (양방향 차압 센서 도입): Pressure Stream 의 `samples[10]` 가 이미 `int16` 이라 양수/음수 모두 표현 가능 → 프로토콜 변경 없이 흡기(음압) 자동 지원. v3.2 (양압 전용 센서) 에선 음수 미사용, v4.0 (XGZP6847A010KPGPN33, ±102 cmH₂O) 부터 음압 영역 활용.
